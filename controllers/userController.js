...
export async function addCart(req, res, next) {
  try {
    ...
    res.status(200).json({
      message: "ajouter un produit au panier de l'utilisateur actuel",
    });
  } catch (err) {
    res.status(400).json({
      message: err.message,
...
export async function getCart(req, res, next) {
  try {
    ...
    res.status(200).json({
      message: "Occurrences de produits récupérées dans le panier de l'utilisateur actuel",
      data: productsArray,
    });
  } catch (e) {
    res.status(400).json({
  ...
  try {
    ...
    if (productIndex === -1) {
      return res.status(404).json({
        error: "Le produit n'a pas été trouvé dans le panier.",
      });
    }
    ...
    await currentUser.save();
    res.status(200).json({
      message: "Produit retiré du panier pour l'utilisateur actuel.",
    });
  } catch (err) {
    res.status(400).json({
      message: err.message,
...
export async function getCartSize(req, res, next) {
  try {
    ...
    res.status(200).json({
      message: "Taille du panier récupérée pour l'utilisateur actuel",
      data: products.size,
    });
  } catch (e) {
    res.status(400).json({
